<!-- BEGIN CONTENT -->
    <div class="page-content-wrapper">
        ...
        <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#modal_add_kategori_produk"> <span class="fa fa-plus"> </span> Add Kategori </button>
        <button class="btn btn-default" onclick="reload_table_kategori_produk()"><i class="fa fa-refresh"></i> Reload</button>
        <br>
        <br>
        <div class="box-body table-responsive">
            <table id="table_kategori_produk" class="table table-bordered table-striped">
...
$(document).ready(function() {
    table4 = $('#table_kategori_produk').dataTable({
        "processing": true,
        "serverSide": true,
        "order": [],
        "ajax": {
            "url": "<?php echo site_url('admin/kategori_produk_list')?>",
            "type": "POST"
        },
        "columnDefs": [
        {
...
function add_kategori_produk(){
    ...
    $('.help-block').empty();
    $('#modal_add_kategori_produk').modal('show');
    $('.modal-title').text('Add Kategori Produk');
}
...
function edit_kategori_produk(id){
    ...
    $('.form-group').removeClass('has-error');
    $('.help-block').empty();
    $('#modal_form_kategori_produk').modal('show');
    $('.modal-title').text('Edit Kategori Produk');

    $.ajax({
        url : "<?php echo site_url('admin/kategori_produk_edit/')?>/" + id,
        type: "GET",
        dataType: "JSON",
...
function reload_table_kategori_produk(){
...
function save_add(){
    ...
    url = "<?php echo site_url('admin/kategori_produk_add')?>";
    ...
    $.ajax({
        ...
        success: function(data)
        {
            if(data.status)
            {
                $('#modal_add_kategori_produk').modal('hide');
                reload_table_kategori_produk();
            }
            $('#btnSave').text('save');
...
function save_edit(){
    ...
    if(save_method == 'add') {
        ...
        url = "<?php echo site_url('admin/kategori_produk_update')?>";
    }
    ...
    $.ajax({
        ...
        success: function(data)
        {
            if(data.status)
            {
                $('#modal_form_kategori_produk').modal('hide');
                reload_table_kategori_produk();
            }
            $('#btnSave').text('save');
            $('#btnSave').attr('disabled',false);
...
function delete_kategori_produk(id_produk){
    if(confirm('Are you sure delete this data?')){
        $.ajax({
            url: "<?php echo site_url('admin/kategori_produk_delete/')?>"+id_produk,
            type: "POST",
            dataType: "JSON",
            success: function(data){
                $('#modal_form_kategori_produk').modal('hide');
...
<div class="modal fade" id="modal_form_kategori_produk" role="dialog">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span>
                </button>
                <h3 class="modal-title">Form Data Kategori Produk</h3>
            </div>
                ...
                <form action="#" id="form" class="form-horizontal">
                    ...
                    <div class="form-body">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-md-9">
                                <input name="category" class="form-control" type="text" style="height: 30px;">
                                <span class="help-block"></span>
                            </div>
                        </div>
                    </div>
...
<div class="modal fade" id="modal_add_kategori_produk" role="dialog">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span>
