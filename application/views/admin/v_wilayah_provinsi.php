        ...
        <div class="page-content">
            ...
            <div class="page-bar">
                <ul class="page-breadcrumb">
                    ...
                    <li>
                        <a href="#">Data Wilayah Provinsi</a>
                    </li>
                </ul>
            ...
            <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#modal_add_wilayah_provinsi"> <span class="fa fa-plus"> </span> Add Wilayah Provinsi </button>
        <button class="btn btn-default" onclick="reload_table_wilayah_provinsi()"><i class="fa fa-refresh"></i> Reload</button>
        <br>
        <br>
        <div class="box-body table-responsive">
            <table id="table_wilayah_provinsi" class="table table-bordered table-striped">
...
$(document).ready(function() {
    table15 = $('#table_wilayah_provinsi').dataTable({ 
        "processing": true, 
        "serverSide": true, 
        "order": [], 
        "ajax": {
            "url": "<?php echo site_url('admin/wilayah_provinsi_list')?>",
            "type": "POST"
        },
        "columnDefs": [
        { 
...
function add_wilayah_provinsi()
{
    ...
    $('.modal-title').text('Add Wilayah Provinsi');
}

function edit_wilayah_provinsi(id)
{
    save_method = 'update';
    $('#form')[0].reset(); 
    $('.form-group').removeClass('has-error'); 
    ...
    $.ajax({
        url : "<?php echo site_url('admin/wilayah_provinsi_edit/')?>/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
            ...
            $('#modal_form_wilayah_provinsi').modal('show');
            $('.modal-title').text('Edit Data Wilayah Provinsi'); 
        },
...
function reload_table_wilayah_provinsi()
{
    table15.api().ajax.reload(null,false);  
}
...
function save_add()
{
    ...
    url = "<?php echo site_url('admin/wilayah_provinsi_add')?>";
    ...
    $.ajax({
        ...
        success: function(data)
        {
            if(data.status) 
            {
                $('#modal_add_wilayah_provinsi').modal('hide');
                reload_table_wilayah_provinsi();
            }
            $('#btnSave').text('save'); 
...
function save_edit()
{
    ...
    if(save_method == 'add') {
        ...
        url = "<?php echo site_url('admin/wilayah_provinsi_update')?>";
    }
    ...
    $.ajax({
        ...
        success: function(data)
        {
            if(data.status) 
            {
                $('#modal_form_wilayah_provinsi').modal('hide');
                reload_table_wilayah_provinsi();
            }
            $('#btnSave').text('save'); 
            $('#btnSave').attr('disabled',false);  
...
function delete_wilayah_provinsi(id)
{
    if(confirm('Are you sure delete this data?'))
    {
        $.ajax({
            url : "<?php echo site_url('admin/wilayah_provinsi_delete')?>/"+id,
            type: "POST",
            dataType: "JSON",
            success: function(data)
            {
...
<div class="modal fade" id="modal_form_wilayah_provinsi" role="dialog">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span>
                </button>
                <h3 class="modal-title">Form Data Wilayah Provinsi</h3>
            </div>
...
<div class="modal fade" id="modal_add_wilayah_provinsi" role="dialog">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span>
